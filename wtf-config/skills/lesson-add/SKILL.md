...
2. 將觀察整合進最相關的既有段落（不新增段落，除非真的無處安放）。
    ...
    **`工具 chat rule`**：
    ...
    - 跑 `python wtf-config/sync_config.py chat-instruction` 重產 `outputs/chat-project-instruction.md`。
    - 完成後提示：「⚠️ 固定規則已更新 — 請複製 outputs/chat-project-instruction.md 全文貼回 claude.ai Project Instruction」
