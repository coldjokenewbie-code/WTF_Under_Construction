  ...
  • 決定執行路徑
    ...
    [Execution Agent] 收到信號開始執行
      ...
      • 完成後寫入 AGENT_SIGNAL.log：DONE|<AgentID>|<修改的主檔案路徑>|<ts>
          │
          ▼ Monitor 自動觸發
    輪流驗收（各 agent 輪流驗，禁自驗自過）
      • 程式碼層：Review diff、scope、編譯
      • 視覺層：/ui-review 擷圖，輪流看圖（最多 10 輪）
      • （選用）Webwright 自動化驗收
          │
     PASS ──► 交 PO 最終驗收（不自行 commit）
     FAIL ──► 退回調整、輪流重驗
```
...
| v2.1 | 2026-06 | 加入強制協作協議：討論 3 輪→共識→執行→輪流驗收（程式碼＋擷圖最多 10 輪）→交 PO 驗收不自行 commit；Tech Lead 自執亦須小隊驗收 |
