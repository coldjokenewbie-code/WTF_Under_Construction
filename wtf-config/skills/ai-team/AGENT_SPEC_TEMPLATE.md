...
**REQUEST 信號**（Tech Lead 寫入 `AGENT_SIGNAL.log`）：
```
REQUEST|[gemini/codex]|_context/AGENT_SPEC_[任務名稱].md|<Timestamp>
```

**Tech Lead 監控指令**（session 開始時啟動一次，常駐整個 session）：
```bash
...
**不要自行決定。**  
將疑問列在完成回報的「待確認」欄位，由 Tech Lead 決定後再繼續。  
寧可擱置，不要猜測。
...
3. **Tech Lead 的 Monitor 收到信號後自動驗收**，不需另行通知
