...
4. **夜間雲端候選篩（僅本步驟，不影響上面 1-3 的正常分流）**：對這筆專案工作，比照 `night-pick` skill 判準逐條檢查——全中才提名，不硬湊，多數單句速記是單一小動作、本來就不夠格，如實跳過：
   ...
   本步驟 `cd "<WTF_ROOT>"` 後分開下指令：`git add missions/QUEUE.md` → `git commit -q -m "inbox: 夜間候選提名 N 筆"` → `git push`（push 被拒 → `git pull --rebase`，衝突 `git rebase --abort` 回報需人工，最多重試 2 次）；與各專案 TaskLog 的 commit 分開跑，QUEUE.md 屬 WTF repo。
...
- **動到 TaskLog 的專案 repo**（各自）：先 `cd "<path>"`，**只 add 本次寫入的 TaskLog 檔**（勿 `add _context/` 或 `-A`，會掃進無關未追蹤檔）——分開下指令（不要用 `git -C` 複合寫法，權限允許清單是照這個分開形狀核准的）：
  `git add "<本次 TaskLog 檔>"` → `git commit -q -m "inbox: 語音待辦 N 筆"` → `git push`。
  push 前若需同步先 `git pull --rebase`；**工作區有既存未提交變更會擋 rebase → 跳過 pull 直接 push**；push 被拒才停下回報，不強推。
