...
**派工鐵律（2026-07-25 改：風險／獨立性判斷，工具中立，不指定模型或成本）：**
...
- 派工必帶三要素：目標與動機、驗收條件、回報格式。各工具依自身模型調度規則選執行者（Claude 見 `model-dispatch.md`；Codex／其他工具依各自轉接層規則），本規則不指定具體模型或以成本為選擇依據。
- 說「已完成」之前必有證據：tool 成功回傳＋驗收逐條對照。**高風險或跨模型的成果**：fresh-context 或跨工具獨立驗證；**低風險單點修改**：允許主 agent 自行執行命令與 read-back，不強制每次另派 agent。檔案交付要經 fresh-context read-back；程式交付要跑測試或實跑。
...
- **數據引用鐵律（2026-08-20 PO 裁定）**：任何統計值（平均、累計、比率、成長率）引用時必須同時標示**統計起訖期間與口徑**（來源、統計範圍、單一標的或合計、單位、絕對值或比率）——沒有區間與口徑的數字，讀者只能猜。**不得自行推定年段或補參數**，那是杜撰口徑。跨年比較須同月同日或同一口徑；時點不一致的數值不得並列成可比較的圖表或欄位。同一份文件內同一指標只用一種口徑，來源衝突時擇一並註明另一口徑與差異原因。**查不到區間或口徑定義的數值，標為未取得並排除**，不得以「大約」「一般認為」帶過。細則與自檢清單見 `playbooks/data-citation.md`。
...
- **存檔預設路徑鐵律（2026-08-24 PO 裁定，2026-08-27 強化）**：凡是做出「可存檔／可寫回」的程式或網頁，檔案選擇器的**預設位置一律指向該檔案應該存放的目標資料夾**，不得讓使用者自己去找路徑。第一次選定後把 handle 存 IndexedDB，之後直接寫回同一位置、不再跳窗。無法預設到精確位置時，至少在按鈕旁明示目標路徑全文。**判準**：使用者若得自己翻資料夾才能存到對的地方，就是沒做完。
  ...
  - 實作寫法與常見失敗見 `playbooks/pitfalls-frontend.md`「檔案存取（FSA）」節。
