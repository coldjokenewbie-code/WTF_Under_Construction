...
- **交付即預覽**（2026-08-20 擴大範圍：不限網頁）：交付任何可看可聽的成品（文件、圖檔、影片、音檔、網頁/HTML/圖表）時，主動用使用者系統預設軟體背景開啟，不用等使用者另外要求。**影片／音檔不可自動播放**：改用「在檔案總管/Finder 顯示」而非直接開啟播放器——macOS 用 `open -R "<檔案>"`、Windows 用 `explorer /select,"<檔案>"`，讓使用者自己點開播放。**批次產出（多個檔案）**：改開啟所在資料夾，不逐一開檔——macOS 用 `/usr/bin/open "<資料夾>"`、Windows 用 `start "" "<資料夾>"`。其餘單一可預覽檔案（文件/圖檔/網頁/圖表）：macOS 一律用絕對路徑 `/usr/bin/open "<檔案>"`（終端機如 cmux 會攔截裸 `open` 開進終端分割視窗，且 cmux 內建瀏覽器版面過於擁擠，不適合預覽）；Windows 用 `start "" "<檔案>"`；無 GUI 環境至少附可點的 `file://`／`http://` 連結，不能只給裸路徑。
