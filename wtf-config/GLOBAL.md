...
- **存檔預設路徑鐵律（2026-08-24 PO 裁定）**：凡是做出「可存檔／可寫回」的程式或網頁，檔案選擇器的**預設位置一律指向該檔案應該存放的目標資料夾**，不得讓使用者自己去找路徑。第一次選定後把 handle 存 IndexedDB，之後直接寫回同一位置、不再跳窗。無法預設到精確位置時，至少在按鈕旁明示目標路徑全文。**判準**：使用者若得自己翻資料夾才能存到對的地方，就是沒做完。實作寫法與常見失敗見 `playbooks/pitfalls-frontend.md`「檔案存取（FSA）」節。
