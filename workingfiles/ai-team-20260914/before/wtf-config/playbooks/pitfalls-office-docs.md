...
## docx 審稿稿的「刪除」有兩種標記，只讀 w:t 會把已刪內容當成缺漏（2026-09-04 CDIC，同型第三次）

**症狀**：拿業主清稿與程式／交付文件比對，跑出一批「清稿有、我方沒有」的差異，看起來像我方漏抄整段文字。實際上那些文字**業主早就刪掉了**，我方沒收錄才是對的。CDIC 本次一口氣誤判 5 項，差點依此「補回」已被業主刪除的內容而污染定稿。

**根因**：審稿方標示刪除有兩種寫法，必須分開偵測——
- **手動刪除線格式**＝`w:rPr/w:strike`（或 `w:dstrike`）：業主直接套刪除線樣式，**文字仍在 `w:t` 裡**。
- **Word 追蹤修訂**＝`w:del`／`w:ins`：文字被包在 `w:del` 內。

只讀 `w:t`（或 python-docx 的 `paragraph.text`）兩種都讀得到，於是已刪除的文字被當成有效正文。實務上**業主多半用刪除線格式、不用追蹤修訂**（CDIC 實測：C 區 0623 清稿 48 處、D 區 9 處、B 區 6 處，`w:del` 一律 0）。

**正解**：讀審稿稿一律逐 `w:r` 走訪，遇 `w:rPr` 含 `strike`／`dstrike` 即整個 run 排除；`w:del` 內容排除、`w:ins` 內容收下。**同一支腳本內對刪除線的處理必須一致**——CDIC 本次的旁證就是同一份清稿另一處刪除線，程式同樣未收錄，報告卻判該列「一致」，可見取文標準前後不一。

**驗法**：先數該檔的 `w:strike`／`w:dstrike`／`w:del`／`w:ins` 各有幾處再開始比對。**數量為 0 才可以放心用簡單取文**；非 0 就必須濾除後再比，並在報告寫明濾了幾處。

**通則**：**比對出「對方有我方沒有」時，先問那段文字在來源文件裡是不是已被刪除**，不要直接判成我方漏抄。同族錯誤 CDIC 已犯三次（2026-07-20 E 區 ESG 條列被誤判漏譯、2026-08-27 B 區 `2_44`「作業」二字漏套刪除線、2026-09-04 D 區 5 項誤判）。
