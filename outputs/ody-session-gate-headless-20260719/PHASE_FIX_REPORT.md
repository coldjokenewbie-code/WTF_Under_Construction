...
## canary 三格全部實測通過
- **user-only**：5/5 穩定 2 收據 include（正式部署情境）。
- **project-only**：codex 階段2（2.1.210）已驗 include 自動簽成。
- **both（user+project 同時 import，4 載入事件並行）**：3/3 穩定 2 有效收據 include——O_EXCL 對 4 事件並行也收斂，project 層額外事件不干擾 user 收據簽成。
- ody-verifier 另獨立跑 3 次 canary，3/3 穩定（複驗 PASS 7/7）。

## 誠實標註（未做）
- 孤兒 bundle GC：本次手動清了 canary 產生的 4f4a；自動 GC（30 天）未實作。
- 未恢復 PreToolUse 強制（PO 手動，見下）。
