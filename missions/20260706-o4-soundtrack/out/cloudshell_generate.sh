...
def token():
    return subprocess.run(["gcloud","auth","print-access-token"],capture_output=True,text=True,check=True).stdout.strip()
TEXTS = {
 "opening_vo": "銀行因「以短支長」的經營模式，較會面臨流動性風險與擠兌威脅。存款保險制度旨在穩定民眾信心，但為防範經營者的道德風險，必須搭配嚴謹的金融監理。回顧歷史，19世紀初期，美國經濟大蕭條導致近萬家銀行倒閉，美國政府為此成立全球首創的聯邦存款保險公司，成為臺灣制度的參考基石。民國70年代，臺灣隨金融自由化風險增加，加上美國伊利諾州大陸銀行危機的國際震撼，促使國內加速推動存保立法。如今，臺灣每人在同一機構最高享有新臺幣300萬元保障，有效化解了過去銀行倒閉、存款可能會損失的歷史夢魘。",
