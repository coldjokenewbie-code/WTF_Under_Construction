...
2. **全域 skills**（Fallback）：專案層沒有的 skill，才從全域路徑載入（實體路徑：/Users/coma/git_folder/WTF_Under_Construction/claude-config/skills/）。
...
**僅適用於 Claude 透過 AGENT_SPEC 明確派發的任務。** 獨立小任務不需回寫。

AGENT_SPEC 中若包含「完成後寫入 AGENT_SIGNAL.log」指示，則：

1. **Agent 完成任務**：寫入 `DONE|<AgentID>|<FileName>|<Timestamp>`。
2. **Claude Code 監聽**：偵測到後自動執行驗收（diff 審查 + 功能確認）。
3. **Claude Code 回寫**：`VERIFIED|<AgentID>|PASS|<Timestamp>`。
4. **驗收失敗**：`VERIFIED|<AgentID>|FAIL|<Timestamp>`，由 Tech Lead 接手修復或退回。
