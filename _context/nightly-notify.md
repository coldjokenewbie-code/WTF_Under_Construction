...
- [ ] 2026-07-05 修訂前項（2026-07-03 SessionStart 提案升級版，採用此版、前項作廢）：提醒式 echo 仍靠模型自覺＝非真自動化。改為**內容注入式**——hook 直接把三檔內容灌進 context，模型無需遵從。腳本正本 `wtf-config/hooks/wtf-session-context.sh`（已含測試與註冊說明）：各機複製到 `~/.claude/` 後在 settings.json 註冊 `"SessionStart": [{"hooks": [{"type": "command", "command": "bash ~/.claude/wtf-session-context.sh"}]}]`。Mac 若已裝提醒版，把 command 換成本行即可。
- [ ] 2026-07-10 nightly 建議確認部署狀態並關閉上方 2026-07-05 項（待用戶核准）
  - `wtf-config/hooks/wtf-session-context.sh`：2026-07-09 commit f49f131 已升級——在三檔制之上額外直接注入 GLOBAL.md+AGENTS.md 全文（解決模型收到「請讀」指示卻不讀的問題）。**Mac 已生效**（commits 由 Mac session 產出）。請確認 **Windows** 是否已同步部署更新後的 `wtf-session-context.sh` 到 `~/.claude/`；若已部署，可將 2026-07-05 項勾 [x] 完成。
